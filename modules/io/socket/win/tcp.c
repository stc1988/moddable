...
void tcpTask(modTimer timer, void *refcon, int refconSize)
{
	...
	int result = select(0, &rfds, &wfds, NULL, &tv);
	if (result > 0) {
		...
		if (FD_ISSET(tcp->skt, &rfds) && (tcp->bytesReadable < kBufferSize)) {
			...
			int bytesRead = recv(tcp->skt, tcp->readBuf + tcp->bytesReadable, kBufferSize - tcp->bytesReadable, 0);
			if (bytesRead > 0) {
...
void xs_listener_constructor(xsMachine *the)
{
	...
	xsTry {
		...
		if (INVALID_SOCKET == listener->skt)
		...
		if (SOCKET_ERROR == bind(listener->skt, (struct sockaddr *)&address, sizeof(address)))
			xsUnknownError("bind socket failed");

		if (SOCKET_ERROR == listen(listener->skt, SOMAXCONN))
...
static void listenerTask(modTimer timer, void *refcon, int refconSize)
{
	...
	while (true) {
		SOCKET skt = accept(listener->skt, NULL, NULL);
