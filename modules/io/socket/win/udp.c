...
void xs_udp_constructor(xsMachine *the)
{
	...
	xsTry {
		...
		if (port) {
			...
			if (SOCKET_ERROR == bind(udp->skt, (struct sockaddr *) &address, sizeof(address)))
...
void udpTask(modTimer timer, void *refcon, int refconSize)
{
	...
	byteLength = recvfrom(udp->skt, data, sizeof(data), 0, (struct sockaddr *)&srcAddr, &srcAddrLen);
