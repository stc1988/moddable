...
/*
	FT3168 capacitive touch driver
	FocalTech FT3168 is register-compatible with the FT5x06/FT6x06 family.
...
class FT3168 {
  ...
  configure(options) {
    ...
    if ('threshold' in options) io.writeUint8(0x80, options.threshold)

    if ('active' in options) io.writeUint8(0x86, options.active ? 0 : 1)

    if ('timeout' in options) io.writeUint8(0x87, options.timeout)
    ...
    if (value) {
      ...
      if ('h' === value) io.flipX = true
      else if ('v' === value) io.flipY = true
      else if ('hv' === value) io.flipX = io.flipY = true
    }

    value = options.length
    if (undefined !== value) io.length = 1 === value ? 1 : 2
  ...
  sample() {
    ...
    const length = Math.min(io.readUint8(0x02) & 0x0f, io.length ?? 2)
    if (0 === length) {
      if (io.none) return
      io.none = true
      return []
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (io.weight) j.weight = data[offset + 4]

      if (io.area) j.area = data[offset + 5] >> 4
