...
#define FT_CONFIG_OPTION_INCREMENTAL
   ...
   * do all of its work.
   *
   * MODDABLE: set by MODDEF_FT_RENDER_POOL_SIZE at the end of this file, since
   * the right size depends on the target. See there.
   */
...
#error "Invalid CFF darkening parameters!"
  ...
  /**************************************************************************
   *
   * MODDABLE: everything below overrides the upstream defaults above.
   *
   * The font engine's build options come from the manifest `defines` section,
   * as MODDEF_CFE_FT_* macros. The defaults here are chosen for a
   * microcontroller: TrueType outlines, no hinting, nothing that needs a
   * filesystem or a compression library.
   *
   *   "defines": {
   *       "cfe": {
   *           "ft": {
   *               "cff": 1,              // .otf and CFF outlines
   *               "hintingBytecode": 1,  // run the TrueType bytecode interpreter
   *               "hintingAuto": 1,      // use the autofitter
   *               "variable": 1,         // OpenType variable font axes
   *               "embeddedBitmaps": 1,  // bitmap strikes inside a font
   *               "gposKern": 1          // kerning from GPOS; follows cfe.kern
   *           }
   *       }
   *   }
   *
   * These are numbers rather than a "hinting": "bytecode" style string because
   * the preprocessor cannot compare strings, and these must select which
   * FreeType source files are compiled.
   */
...
#ifndef MODDEF_CFE_FT_CFF
	#define MODDEF_CFE_FT_CFF (0)
#endif
#ifndef MODDEF_CFE_FT_VARIABLE
	#define MODDEF_CFE_FT_VARIABLE (0)
#endif
#ifndef MODDEF_CFE_FT_EMBEDDEDBITMAPS
	#define MODDEF_CFE_FT_EMBEDDEDBITMAPS (0)
#endif
#ifndef MODDEF_CFE_FT_HINTINGBYTECODE
	#define MODDEF_CFE_FT_HINTINGBYTECODE (0)
#endif
#ifndef MODDEF_CFE_FT_HINTINGAUTO
	#define MODDEF_CFE_FT_HINTINGAUTO (0)
#endif
/*
	A manifest may write an option as a JSON boolean, which reaches the
	preprocessor as the word `true`. In C before C23 that word is neither a
	macro nor a keyword here -- <stdbool.h> has not been included this early --
	so `#if` would quietly read it as 0, and an option set to `true` would look
	like one that was never set.

	So give it a value for as long as it takes to read the options below, and
	only where the language does not: C23 and C++ define it themselves, and are
	left alone. What is defined here is undefined again a few lines down, so
	nothing that includes FreeType sees it.

	`false` needs no such help -- an unknown identifier already reads as 0 --
	but it goes with `true` rather than leaving half a definition behind.
*/
#if !defined(true) && !defined(__cplusplus) && (!defined(__STDC_VERSION__) || (__STDC_VERSION__ < 202311L))
	#define true 1
	#define false 0
	#define MODDABLE_FT_UNDEF_BOOLEANS
#endif

/*
	Kerning from GPOS follows the kerning option unless it is asked for by name,
	since a font that kerns only from GPOS is otherwise silently unkerned.
*/
#ifndef MODDEF_CFE_FT_GPOSKERN
	#if MODDEF_CFE_KERN
		#define MODDEF_CFE_FT_GPOSKERN (1)
	#else
		#define MODDEF_CFE_FT_GPOSKERN (0)
	#endif
#endif

#ifdef MODDABLE_FT_UNDEF_BOOLEANS
	#undef true
	#undef false
	#undef MODDABLE_FT_UNDEF_BOOLEANS
#endif

  /* no filesystem, no compression libraries, no external dependencies */
...
#undef FT_CONFIG_OPTION_ERROR_STRINGS
  ...
  /* colored glyph layers and SVG glyphs are not rendered by the font engine */
#undef TT_CONFIG_OPTION_COLOR_LAYERS

#if !MODDEF_CFE_FT_EMBEDDEDBITMAPS
	#undef TT_CONFIG_OPTION_EMBEDDED_BITMAPS
#endif

  /*
	Glyph names cost flash and nothing in the SDK looks a glyph up by name, so
	they go with TrueType alone. They cannot go when CFF is built: that driver
	needs the PostScript names service, and ttcmap.c guards its call to
	tt_face_get_ps_name with FT_CONFIG_OPTION_POSTSCRIPT_NAMES while ttpost.c
	compiles the function under TT_CONFIG_OPTION_POSTSCRIPT_NAMES -- so turning
	one on without the other does not link.
  */
...
#if !MODDEF_CFE_FT_VARIABLE
	#undef TT_CONFIG_OPTION_GX_VAR_SUPPORT
#endif

/*
	The pool the scan-line converter works in, which is statically allocated
	(see gray_convert_glyph in ftgrays.c) and so a fixed cost in every build.

	An outline whose cells do not fit is rendered in bands, and each band walks
	the outline again -- so a pool that is too small for what a project draws
	costs time on every frame. On an ESP32-S3 drawing the shapes example, 8192
	costs about 25% more CPU than the default does.

	Lower it on a target where the memory matters more than the time:

	  "defines": {
	      "ft": { "render_pool_size": 8192 }
	  }

	The value is compared with `#if` in ftgrays.c, so it must be an integer
	constant expression -- no casts.
*/
#ifndef MODDEF_FT_RENDER_POOL_SIZE
	#define MODDEF_FT_RENDER_POOL_SIZE (16384)
#endif
#undef FT_RENDER_POOL_SIZE
#define FT_RENDER_POOL_SIZE  MODDEF_FT_RENDER_POOL_SIZE

/*
	Kerning from the GPOS table, which most OpenType fonts use in place of the
	legacy `kern` table -- and CFF fonts almost always do. Without this, such a
	font kerns not at all, silently. The code is compiled either way, as part of
	the sfnt amalgamation; this only decides whether it does anything.

	This is FreeType's own basic implementation of the GPOS `kern` feature, not
	a shaper: it reads kerning pairs whose value format is exactly one x advance
	and nothing more. A variable font stores kerning with a device or variation
	table alongside the advance, which this skips -- so such a font still does
	not kern. The GPOS table itself is read in place from the font data, so no
	part of this costs RAM.
*/
#if MODDEF_CFE_FT_GPOSKERN
	#define TT_CONFIG_OPTION_GPOS_KERNING
#endif
