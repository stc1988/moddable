...
#define SCALED( x )  ( (x) * ( 1L << shift ) - delta )
  ...
  /* MODDABLE: FT_Outline_Render and FT_Outline_Get_Bitmap are the only functions  */
  /* in this file that reference the renderer layer (FT_Lookup_Renderer), which    */
  /* pulls in ftobjs.c and everything below it. Neither the Commodetto outline     */
  /* module nor the font engine calls them -- both drive ft_grays_raster directly  */
  /* -- so they are omitted to keep builds without the object layer small.         */
#ifdef MODDABLE_FT_OUTLINE_RENDER
...
#endif /* MODDABLE_FT_OUTLINE_RENDER */
