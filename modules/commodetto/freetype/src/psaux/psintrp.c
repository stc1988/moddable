...
#define PS_STORAGE_SIZE 3
    ...
    CF2_GlyphPath  glyphPath = NULL;		/* MODDABLE: on the heap, see below */
    ...
    /*
      MODDABLE: CF2_GlyphPathRec holds three hint maps and is the largest thing
      this function needs -- around 16K, which upstream puts on the stack. That
      is more stack than a microcontroller task has, and it made rendering any
      OpenType/CFF glyph overflow. It comes from the heap instead, as the
      rasterizer's cell pool does, so no application has to size its stack for
      it. One allocation per glyph is nothing beside rasterizing one.
    */
    glyphPath = (CF2_GlyphPath)memory->alloc( memory, sizeof ( CF2_GlyphPathRec ) );
    if ( !glyphPath )
    {
  ...
  exit:
    ...
    if ( glyphPath )
    {
      ...
      memory->free( memory, glyphPath );		/* MODDABLE */
    }
