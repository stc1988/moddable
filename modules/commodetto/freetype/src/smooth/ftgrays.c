...
#if ESP32 || nrf52
  #include "xsPlatform.h"		/* MODDABLE: ICACHE_RAM_ATTR, c_malloc */
#endif
...
#define SCALED( x )  ( (x) * ( 1L << shift ) - delta )
  ...
  {
    /* MODDABLE: upstream puts the cell pool on the stack. That is far too much   */
    /* stack for a microcontroller, so the pool is static instead. Not thread     */
    /* safe, which is fine here: rendering happens on one task.                   */
#if ESP32 && kCPUESP32
    static ICACHE_RAM_ATTR TCell  buffer[FT_MAX_GRAY_POOL];
#elif nrf52
    static TCell    *buffer;		/* allocated on first use -- see below */
#else
    static TCell    buffer[FT_MAX_GRAY_POOL];
#endif
...
#if nrf52
    /* MODDABLE: no static pool on nrf52 -- allocate it once, on first render. */
    if ( !buffer )
    {
      buffer = c_malloc( sizeof ( TCell ) * FT_MAX_GRAY_POOL );
      if ( !buffer )
        return Smooth_Err_Out_Of_Memory;
    }
#endif
