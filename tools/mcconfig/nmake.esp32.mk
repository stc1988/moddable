...
BUILD_CMD = python %IDF_PATH%\tools\idf.py $(IDF_PY_LOG_FLAG) build
BUILD_MSG =
...
!ELSE
START_SERIAL2XSBUG = echo Launching app... & echo Type Ctrl-C twice after debugging app. && set "XSBUG_PORT=$(XSBUG_PORT)" && set "XSBUG_HOST=$(XSBUG_HOST)" && $(BUILD_DIR)\bin\win\release\serial2xsbug $(USB_VENDOR_ID):$(USB_PRODUCT_ID) $(DEBUGGER_SPEED) 8N1 -dtr
!ENDIF
...
!IF "$(XSBUG_LOG)"=="1"
!IF "$(USE_USB)"=="0"
START_SERIAL2XSBUG = echo Launching app... && set "XSBUG_LOG_PORT=$(XSBUG_LOG_PORT)" && set "XSBUG_PORT=$(XSBUG_PORT)" && set "XSBUG_HOST=$(XSBUG_HOST)" && set "XSBUG_PROJECT=$(MAIN_DIR)" && cd $(MODDABLE)\tools\xsbug-log && node xsbug-log start /B $(BUILD_DIR)\bin\win\release\serial2xsbug $(PORT_TO_USE) $(DEBUGGER_SPEED) 8N1
!ELSE
START_SERIAL2XSBUG = echo Launching app... && set "XSBUG_LOG_PORT=$(XSBUG_LOG_PORT)" && set "XSBUG_PORT=$(XSBUG_PORT)" && set "XSBUG_HOST=$(XSBUG_HOST)" && set "XSBUG_PROJECT=$(MAIN_DIR)" && cd $(MODDABLE)\tools\xsbug-log && node xsbug-log start /B $(BUILD_DIR)\bin\win\release\serial2xsbug $(USB_VENDOR_ID):$(USB_PRODUCT_ID) $(DEBUGGER_SPEED) 8N1
!ENDIF
START_XSBUG =
!ENDIF

IDF_RECONFIGURE_CMD=python %IDF_PATH%\tools\idf.py $(IDF_PY_LOG_FLAG) reconfigure -D mxDebug=$(DEBUG) -D TMP_DIR="$(TMP_DIR)" -D SDKCONFIG_DEFAULTS=$(SDKCONFIG_FILE) -D SDKCONFIG_HEADER="$(SDKCONFIG_H)" -D CMAKE_MESSAGE_LOG_LEVEL=$(CMAKE_LOG_LEVEL) -D DEBUGGER_SPEED=$(DEBUGGER_SPEED) -D IDF_TARGET=$(ESP32_SUBCLASS) $(IDF_BUILD_OPTIONS)
...
INC_DIRS = \
	...
	$(MANAGED_COMPONENT_DIRS) \
 	...
 	-I$(IDF_PATH)\components\bt\porting\include \
...
precursor: idfVersionCheck $(BLE) dependencies $(SDKCONFIG_H) $(LIB_DIR) $(BIN_DIR)\xs_$(ESP32_SUBCLASS).a
	fc /b $(BIN_DIR)\xs_$(ESP32_SUBCLASS).a $(BLD_DIR)\xs_$(ESP32_SUBCLASS).a > nul 2>&1 || copy $(BIN_DIR)\xs_$(ESP32_SUBCLASS).a $(BLD_DIR)\.
...
$(SDKCONFIG_H): $(SDKCONFIG_FILE) $(TMP_DIR)\xs_idf_components.txt
