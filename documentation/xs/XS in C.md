...
The XS chunk heap is compacted during garbage collection to optimize memory available to scripts. This compaction can invalidate pointers to structures in the chunk heap, including strings and buffers. XS in C code should refresh pointers to these structures after invoking any API that can trigger a garbage collection. 

The garbage collector may run any time a slot or chunk allocation is performed. This can happen in calls that set a property, but it can also happen during type coercion or a get, because of proxy traps.

The following sections enumerate the functions and macros that have the potential to invoke the garbage collector and consequently invalidate pointers into the chunk heap.

##### Memory Allocation and Instantiation
Any macro that creates a new object, string, or allocates buffer allocates may cause a garbage collection.
* **Strings**: `xsString`, `xsStringBuffer`, `xsmcSetString`, `xsmcSetStringBuffer`
* **Objects and Arrays**: `xsNewObject`, `xsmcNewObject`, `xsmcSetNewObject`, `xsNewArray`, `xsmcSetNewArray`, `xsNewHostInstance`, `xsmcNewHostInstance`, `xsmcSetNewHostInstance`
* **Host Entities**: `xsNewHostFunction`, `xsNewHostConstructor`, `xsNewHostObject`
* **Buffers and Chunks**: `xsArrayBuffer`, `xsmcSetArrayBuffer`, `xsArrayBufferResizable`, `xsmcSetArrayBufferResizable`, `xsSetArrayBufferLength`, `xsmcSetArrayBufferLength`, `xsSetHostChunk`, `xsmcSetHostChunk`, `xsmcSetHostBuffer`

##### JavaScript Callbacks
Calling JavaScript functions or constructors runs arbitrary JS code, which may trigger garbage collection.
* **Function Calls**: `xsCall0` through `xsCall8`, `xsmcCall`, `xsCallFunction0` through `xsCallFunction8`
* **Instantiation**: `xsNew0` through `xsNew8`, `xsmcNew`, `xsNewFunction0` through `xsNewFunction2`, `xsCall0_noResult` through `xsCall8_noResult`, `xsmcCall_noResult`

##### Property Accessors and Proxies
Any function that may invoke a proxy trap may trigger garbage collection because the proxy trap can run JavaScript code.
* **Getting Properties**: `xsGet`, `xsGetAt`, `xsGetIndex`, `xsmcGet`, `xsmcGetAt`, `xsmcGetIndex`
* **Setting Properties**: `xsSet`, `xsSetAt`, `xsSetIndex`, `xsmcSet`, `xsmcSetAt`, `xsmcSetIndex`
* **Checking Properties**: `xsHas`, `xsHasAt`, `xsHasIndex`, `xsmcHas`, `xsmcHasAt`, `xsmcHasIndex`
* **Deleting Properties**: `xsDelete`, `xsDeleteAt`, `xsmcDelete`, `xsmcDeleteAt`
* **Defining Properties**: `xsDefine`, `xsDefineAt`, `xsmcDefine` (can invoke `defineProperty` trap)
* **Enumeration**: `xsEnumerate` (can invoke `ownKeys` trap)

##### Coercions and Type Conversions
Coercing a JavaScript object to a primitive type can invoke its `valueOf()` or `toString()` methods. Converting to a string requires allocating a new string. These methods may also invoke JavaScript coercion functions.
* **To Number/Integer**: `xsToInteger`, `xsmcToInteger`, `xsToUnsigned`, `xsmcToUnsigned`, `xsToNumber`, `xsmcToNumber`
* **To String**: `xsToString`, `xsmcToString`, `xsToStringBuffer`, `xsmcToStringBuffer`. Note that calling "To String" functions a second time on the same slot will never perform a conversion since the value is already a string. This means that "To String" functions may be safely called to refresh a pointer to a string that has already been coerced.
* **To Identifier**: `xsToID` 
##### Throwing Errors
Throwing an exception creates a new error instance which can trigger garbage collection. While the throw functions don't return, code that catches them in an `xsCatch` block must assume any chunk pointers are invalid.
* **Exceptions**: `xsUnknownError`, `xsEvalError`, `xsRangeError`, `xsReferenceError`, `xsSyntaxError`, `xsTypeError`, `xsURIError`, `xsErrorPrintf`
