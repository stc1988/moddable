...
| :---: | :--- | :--- | :--- |
| <img src="./../assets/devices/moddable-one.png" width=125><BR>Moddable One | `esp/moddable_one`<BR>`simulator/moddable_one` | **2.4" IPS display**<BR>240 x 320 QVGA<BR>16-bit color<BR>Capacitive touch<BR><BR>14 External pins | <li>[Moddable One Developer Guide](./moddable-one.md)</li><li>[Moddable product page](https://www.moddable.com/hardware)</li> |
| <img src="./../assets/devices/moddable-display-1.png" height=125><BR>Moddable Display 1 | `esp/moddable_display_1`<BR>`simulator/moddable_one` | **2.4" IPS display**<BR>240 x 320 QVGA<BR>16-bit color<BR>Capacitive touch<BR><BR>14 External pins | <li>[Moddable Display Developer Guide](./moddable-display.md)</li><li>[Moddable product page](https://www.moddable.com/hardware)</li> |
| <img src="./../assets/devices/esp8266.png" width=125><BR>Node MCU ESP8266 | `esp/nodemcu`<BR>`simulator/nodemcu` | | |
| <img src="./../assets/devices/moddable-three.png" width=125><BR>Moddable Three | `esp/moddable_three`<BR>`simulator/moddable_three` |  **2.13" ePaper display**<BR>250 x 122 pixels<BR>1-bit black and white<BR><BR>11 External pins | <li>[Moddable Three Developer Guide](./moddable-three.md)</li><li>[Moddable product page](https://www.moddable.com/hardware)</li> |
| <img src="./../assets/devices/moddable-display-3.png" width=125><BR>Moddable Display 3 | `esp/moddable_display_3`<BR>`simulator/moddable_three` |  **2.13" ePaper display**<BR>250 x 122 pixels<BR>1-bit black and white<BR><BR>11 External pins | <li>[Moddable Display Developer Guide](./moddable-display.md)</li><li>[Moddable product page](https://www.moddable.com/hardware)</li> |
| <img src="https://www.moddable.com/assets/images/moddable-zero-sm.jpg" width=125><BR>Moddable Zero | `esp/moddable_zero` |  | <li>[Moddable Zero Developer Guide](./moddable-zero.md)</li> |
...
8. Install [esptool](https://github.com/espressif/esptool). esptool 5.0 or newer is required. We've used [brew](https://brew.sh/):

	```text
	brew install esptool
	```

	Installing from [PyPI](https://pypi.org/project/esptool/) works equally well, if you prefer:

	```text
	pipx install esptool
	```

	Either way, `esptool` must be on your `PATH`. Confirm the version with `esptool version`.
...
4. Install `esptool`, which builds the firmware image and flashes it to the device. esptool 5.0 or newer is required.
...
4. Download and unzip the [Cygwin toolchain support package](https://github.com/Moddable-OpenSource/tools/releases/download/v1.0.0/cygwin.win32.zip). Copy the `cygwin` directory into the `esp` directory.

5. Download and unzip the [Xtensa lx106 architecture GCC toolchain](https://github.com/Moddable-OpenSource/tools/releases/download/v1.0.0/esp8266.toolchain.win32.zip). Copy the `xtensa-lx106-elf` directory into the `esp` directory.
...
8. Download and run the [Python installer](https://www.python.org/downloads/windows/) for Windows. Python 3.10 or newer is required. On the first screen of the installer, check "Add python.exe to PATH".

9. Open a "Command Prompt" window and install [esptool](https://github.com/espressif/esptool), which builds the firmware image and flashes it to the device. esptool 5.0 or newer is required.

	```text
	pip install esptool
	```

	This installs `esptool.exe` into your Python `Scripts` directory and brings in `pyserial` with it. Confirm that `esptool` is on your `PATH` and new enough:

	```text
	esptool version
	```

10. Connect the ESP8266 to your computer with a USB cable.

11. Launch the Windows Device Manager, open the "Ports (COM & LPT)" section, and verify the "Silicon Labs CP210x USB to UART Bridge" is displayed. Note the COM port (e.g. COM3) for the next step.

	> The Device Manager interface may vary depending on the Windows OS version.

12. Open the "Environment Variables" dialog of the Control Panel app by following [these instructions](https://www.architectryan.com/2018/08/31/how-to-change-environment-variables-on-windows-10/). From that dialog:
	- Create a User Variable called `BASE_DIR` and set it to `%USERPROFILE%
		- Variable name: `BASE_DIR`
		- Variable value: `%USERPROFILE%`
	- Create a User Variable called `UPLOAD_PORT` and set it to the COM port you identified in Step 11
...
13. Launch the "x86 Native Tools Command Prompt for VS 2019" command line console. Verify the setup by building `helloworld` for your device target:
...
4. Install `esptool`. esptool 5.0 or newer is required, running on Python 3.10 or newer; see Step 8 of the [installation instructions](#win-instructions) above.
...
6. Install [esptool](https://github.com/espressif/esptool), which builds the firmware image and flashes it to the device. esptool 5.0 or newer is required. We've used [pipx](https://pipx.pypa.io/), which keeps it isolated from the system Python:
	...
	Your distribution may also package `esptool`, but distribution packages are often older than 5.0. Whichever you use, `esptool` must end up on your `PATH`. Confirm the version with:
...
4. Install `esptool`. esptool 5.0 or newer is required.
